...
def load_long_csv_from_text(text: str) -> pd.DataFrame:
    # Robust: skip empty lines, keep commas inside quotes
    df = pd.read_csv(
        StringIO(text),
        header=None,
        names=["respondent_id", "question_text", "item", "answer"],
        skip_blank_lines=True
    )
    df["respondent_id"] = pd.to_numeric(df["respondent_id"], errors="coerce").astype("Int64")
    # Normalize whitespace
    for c in ["question_text", "item", "answer"]:
        df[c] = df[c].astype(str).str.strip()
        df.loc[df[c].isin(["nan", "None"]), c] = ""
    return df.dropna(subset=["respondent_id"]).reset_index(drop=True)
